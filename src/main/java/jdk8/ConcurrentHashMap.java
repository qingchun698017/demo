...
//     * 首先计算key的hash值来确定该元素放在数组的哪个位置
//     * 然后遍历该位置的所有节点，如果不存在的话返回null
...
//                    return e.val;
//            }
//            //hash值为负值表示正在扩容，这个时候查的是ForwardingNode的find方法来定位到
//            // nextTable来查找，查找到就返回
//            else if (eh < 0)
...
//        int binCount = 0;
//        /**
//         * 这边加了一个循环，就是不断的尝试，因为在table的初始化和casTabAt用到了
//         * compareAndSwapInt、compareAndSwapObject
//         * 如果其他线程正在修改tab，那么尝试就会失败，所以这边要加一个for循环，不断的尝试
//         */
...
//                /**
//                 * 根据hash值计算得到数组索引i，如果tab[i]为空，使用CAS进行添加（链表的头结点），
//                 * 创建一个Node添加到数组中去，null表示的是下一个节点为空
//                 * 添加失败则进入下次循环。。
...
//            else {
//                V oldVal = null;
//                //如果在这个位置有元素的话，就采用synchronized的方式加锁，锁住表头
//                synchronized (f) {
//                    if (tabAt(tab, i) == f) {
//                        if (fh >= 0) {
//                            binCount = 1;
...
//                            }
//                        }
//                        //若为红黑树，则调用putTreeVal方法，将该元素添加到树中去
...
//            if ((sc = sizeCtl) < 0)
//                Thread.yield(); //如果sizeCtl小于0，说明别的数组正在进行初始化，则让出执行权
//                //SIZECTL：表示当前对象的内存偏移量，sc表示期望值，-1表示要替换的值，设定为-1表示要初始化表了
//            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
//                try {
//                    if ((tab = table) == null || tab.length == 0) {
//                        //创建指定大小的Node数组，否则创建指定默认大小(16)的Node数组
...
//         * 如果给定的大小大于等于数组容量最大的一半，则直接使用最大容量，
//         * 否则使用tableSizeFor算出来
...
//        //是否继续向前查找的标志位，当advance == true时，表明该节点已经处理过了 跳过该节点
//        boolean advance = true;
//        // to ensure sweep(清扫) before committing nextTab,在完成之前重新在扫描一遍数组，看看有没完成的
...
//            // f.hash == -1 表示遍历到了ForwardingNode节点，意味着该节点已经处理过了，跳过
//            // 这里是控制并发扩容的核心
//            else if ((fh = f.hash) == MOVED)
